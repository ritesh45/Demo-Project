...
public class Mycontroller {
	...
	public Employee getEmployeeById(@PathVariable int id) throws Exception {
		...
		if (!emp.isPresent())
			throw new RecordNotFoundException("Could not find employee with id- " + id);

		return emp.get();
	}

	@RequestMapping(value = "/employee/add", method = RequestMethod.POST)
	public ResponseEntity<Object> createEmployee(@Valid @RequestBody Employee newemp) {
		System.out.println(this.getClass().getSimpleName() + " - Create new employee method is invoked.");
		Employee use = service.addNewEmployee(newemp);
		URI uri = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}").buildAndExpand(use.getId()).toUri();
